var engine = require('engine.io'),
    sockets = Object.create(null),
    rooms = Object.create(null);

function addToRoom(socket, id) {
    if (!rooms[id]) {
        rooms[id] = [];
    }
    if (~rooms[id].indexOf(socket)) {
        return;
    }
    console.log('add', id, socket.id);
    rooms[id].push(socket);
    sockets[socket.id] = id;
    broadcast(socket, {roomMembers: rooms[id].length});
}

function removeFromRooms(socket) {
    var id = sockets[socket.id],
        room = rooms[id];

    if (!room) {
        return;
    }

    console.log('remove', id);
    room.splice(room.indexOf(socket), 1);
}

function send(socket, message) {
    socket.send(JSON.stringify(message));
}

function broadcast(socket, message) {
    var id = sockets[socket.id];

    if (!id) {
        return;
    }
    message.socketId = socket.id;

    rooms[id].forEach(function (item, i) {
    	console.log(item === socket, i);
        if (item === socket) {
            return;
        }
        send(item, message);
    });
}

module.exports = function (http) {
    var server = engine.attach(http);

    server.on('connection', function (socket) {
        socket.on('message', function(data){
            try {
                data = JSON.parse(data);
            } catch(e) {}

            if (data.joinRoom) {
                addToRoom(socket, data.joinRoom);
            } else if (data.leftRoom) {
                removeFromRooms(socket, data.joinRoom);
            } else {
            	/*console.log(data);*/
                broadcast(socket, data);
            }

        });
        socket.on('close', function () {
        	console.log('disconnect');
            broadcast(socket, {disconnected: true});
            removeFromRooms(socket);
        });
    });
};
